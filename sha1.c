...
static inline WORD32 ROTL(int n, WORD32 x)
{
	asm("roll %%cl,%0" :"=r" (x) :"0" (x),"c" (n));
	return x;
}
#else
#warning -------------------------NO ASM ROTATIONS----------------------
...
#define expand(W,i) \
	( W[i & 15] = MASK32(ROTL(1, (W[i & 15] ^ W[i - 14 & 15] ^ \
									W[i - 8 & 15] ^ W[i - 3 & 15]))) )
...
PSHA1CTX CRYPTPAK_API SHA1_Create()
{
    ...
    PSHA1CTX pCtx = malloc(sizeof(SHA1CTX));
   ...
   PSHA1CTX pCtx )
{
    ...
    *pData++ = 0x80;
	...
	copyToBWORD32(pCtx->data, pCtx->dataBuffer, SHA1_DATASIZE);
...
/* @(#) enciu -- test vectors from FIPS PUB 180-1 */

// the selftest strings and digests
static const char* selfTestSource[2] =
{
  "abc",
  "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"
};

static const WORD8 selfTestDigest[2][SHA1_DIGESTSIZE] = {
...
BYTEBOOL CRYPTPAK_API SHA1_SelfTest()
{
	...
	for (nI = 0; nI < 2; nI++)
	{
		...
		/* @(#) enciu -- fixed a bug; wasnt computing test digest correctly :\ */
		SHA1_Update(pCtx, selfTestSource[nI], strlen(selfTestSource[nI]));
		SHA1_Final(actDigest, pCtx);
		SHA1_Destroy(pCtx);

		if ( memcmp((WORD8*) &selfTestDigest[nI][0], actDigest, SHA1_DIGESTSIZE) )
        		return BOOL_FALSE;
	}
